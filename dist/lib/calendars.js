...
class CalendarSystem {
    ...
    timestampToCalendar(timestamp) {
        ...
        for (const cycle of cycles) {
            ...
            if (cycle.subdivisions) {
                const subdivResult = this.processSubdivisions(cycle.subdivisions, cycleResult.remaining, result, cycle.id);
                Object.assign(result, subdivResult.data);
                remaining = subdivResult.remaining;
            }
        }
        if (elapsed < 0)
            result[cycles[0].id] -= 1;
    ...
    getOverride(source, index) {
        if (!source || !source.exceptions || index === undefined)
            return undefined;
    ...
    calendarToTimestamp(calendarData) {
        ...
        for (const cycle of cycles) {
            ...
            if (cycle.subdivisions) {
                timestamp += this.calculateSubdivisionTime(cycle.subdivisions, { ...calendarData, [`${cycle.id}_index`]: count }, cycle.id);
            }
        }
        return timestamp;
    }
